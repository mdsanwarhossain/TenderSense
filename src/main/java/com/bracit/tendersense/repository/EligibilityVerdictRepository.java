...
public interface EligibilityVerdictRepository extends JpaRepository<EligibilityVerdict, Long> {
    ...
    Optional<EligibilityVerdict> findByTenderId(Long tenderId);
     ...
     */
    @Query("select v from EligibilityVerdict v left join fetch v.gaps where v.tender.id = :tenderId")
    Optional<EligibilityVerdict> findByTenderIdWithGaps(@Param("tenderId") Long tenderId);

    /**
     * Status plus blocking-gap count for a whole page of tenders, in one query.
     *
     * <p>The shortlist needs only these two facts, so loading full verdict graphs per
     * row would be both an N+1 and a lazy-loading hazard.
     */
    @Query("""
